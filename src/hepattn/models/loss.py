...
def object_bce_costs(pred_logits, target_labels):
    """
...
def object_ce_costs(pred_logits, target_labels):
    """
...
def mask_dice_loss(pred_logits, true, mask=None, weight=None, eps=1):
    ...
    numerator = 2 * (preds * true)
    denominator = preds + true

    if weight is not None:
        numerator *= weight
        denominator *= weight

    loss = 1 - (numerator.sum(-1) + eps) / (denominator.sum(-1) + eps)
...
def mask_dice_costs(pred_logits, true, eps=1):
    with torch.autocast(device_type="cuda", enabled=False):
        ...
        numerator = 2 * torch.einsum("bnc,bmc->bnm", preds, true)
        # Compute the denominator using sum over the last dimension (C) and broadcasting
        denominator = preds.sum(-1).unsqueeze(2) + true.sum(-1).unsqueeze(1)
...
def kl_div_loss(pred_logits, true, mask=None, weight=None, eps=1e-6):  # noqa: ARG001
...
# Context manager necessary to overwride global autocast to ensure float32 cost is returned
# @torch.autocast(device_type="cuda", enabled=False)
# def kl_div_costs(pred_logits, true, eps=1e-6):
